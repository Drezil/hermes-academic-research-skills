...
Configuration (`claim_audit_config` block in `hermes-academic-pipeline/SKILL.md` mode flags):
...
When `gold_set_path` is non-null, the agent enters calibration mode (modeled on `hermes-academic-paper-reviewer/references/calibration_mode_protocol.md` and detailed in `hermes-academic-pipeline/references/claim_audit_calibration_protocol.md`).
