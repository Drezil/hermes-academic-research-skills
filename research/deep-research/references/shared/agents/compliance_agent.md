...
The orchestrator (or standalone skill) passes the input contract via the Agent tool with `model: sonnet` or higher (per model tiering policy). The agent returns the serialised compliance_report. The orchestrator validates against Schema 12 before appending to passport.
