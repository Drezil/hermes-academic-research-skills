...
=== Step 0: RESEARCH READINESS CHECK ===
     ...
     +-> [socratic_mentor_agent] -> Confirm what materials the user already has
         ...
         -> If research foundation is lacking, recommend running hermes-deep-research (socratic mode) first
     |
...
=== Step 2.5: CONTRIBUTION SHARPENING ===
     ...
     +-> [socratic_mentor_agent] -> Ask the user to articulate the contribution their
         ...
         (single-sourced in hermes-deep-research/agents/socratic_mentor_agent.md Layer 5 —
...
-> Or use hermes-academic-paper-reviewer to review the Chapter Plan
```
...
**Single source of the questions.** Step 2.5 asks the later-stage anchored forms **L5-W1 / L5-W2 / L5-W3** defined under Layer 5 (SIGNIFICANCE & CONTRIBUTION) in `hermes-deep-research/agents/socratic_mentor_agent.md` — read the question text there. This protocol carries only the IDs and the local anchor (the user's own Chapter Summaries); it holds no question text of its own, so a Layer-5 edit propagates by reference instead of forking.
