...
When a tiering direction is active, route repeated same-stage calls to the SAME worker so its cache accumulates — e.g. across the Stage 3 → 3' review loop, the re-dispatched roles (the narrow re-review team: `eic`, `editorial_synthesizer` — not the full panel, and NOT `field_analyst`: on the normal pipeline path it is not re-invoked at Stage 3' — the Round-1 Reviewer Configuration Cards are passed forward as data; the sole exception is the visibly-marked regeneration fallback when the cards are unavailable, per `hermes-academic-paper-reviewer/references/re_review_mode_protocol.md` § Yardstick Continuity) should reuse their Stage 3 workers rather than spawning fresh ones per round. The reuse rule is tier-independent and covers both judgment-type roles identically. A fresh worker per call re-pays the full context write and can erase the tiering savings entirely. With the flag unset this guidance imposes nothing: default behavior stays byte-equivalent, dispatch shapes included.
...
| Skill | Agents |
|---|---|
| hermes-deep-research (10) | `socratic_mentor`, `research_question`, `research_architect`, `synthesis`, `devils_advocate`, `editor_in_chief`, `ethics_review`, `risk_of_bias`, `meta_analysis`, `source_verification` |
| hermes-academic-paper (6) | `socratic_mentor`, `argument_builder`, `structure_architect`, `peer_reviewer`, `revision_coach`, `literature_strategist` |
| hermes-academic-paper-reviewer (6) | `eic`, `methodology_reviewer`, `domain_reviewer`, `perspective_reviewer`, `devils_advocate_reviewer`, `editorial_synthesizer` (mechanical by v3.6.2 design but emits the final decision letter — judgment-type conservatively until data says otherwise) |
| hermes-academic-pipeline (3) | `pipeline_orchestrator`, `claim_ref_alignment_audit`, `integrity_verification` |
...
| Skill | Agents |
|---|---|
| hermes-deep-research (4) | `bibliography` (citation existence is handled by the deterministic verification gate, so the lookup layer does not depend on this agent's tier), `timeline_extraction`, `report_compiler`, `monitoring` |
| hermes-academic-paper (6) | `intake`, `draft_writer` (highest-savings / most quality-sensitive — see Direction 2), `abstract_bilingual`, `citation_compliance`, `visualization`, `formatter` (STAMP-ONLY by design) |
| hermes-academic-paper-reviewer (1) | `field_analyst` |
| hermes-academic-pipeline (2) | `collaboration_depth` (advisory-only, never blocks), `state_tracker` |
