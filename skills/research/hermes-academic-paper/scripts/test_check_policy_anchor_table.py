...
class CheckPolicyAnchorTableNatureSourceOfTruthTest(unittest.TestCase):
    ...
    def test_nature_dedup_integration_on_real_files(self) -> None:
        anchor_path = REPO_ROOT / "hermes-academic-paper/references/policy_anchor_table.md"
        venue_path = REPO_ROOT / "hermes-academic-paper/references/venue_disclosure_policies.md"
    ...
    def test_main_command_invokes_dedup_helper(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            tdir = Path(td)
            anchor = REPO_ROOT / "hermes-academic-paper/references/policy_anchor_table.md"
            venue = REPO_ROOT / "hermes-academic-paper/references/venue_disclosure_policies.md"
            # Build a working subtree mirror without the canonical Nature source
            # to confirm main() surfaces the dedup violation.
            (tdir / "hermes-academic-paper/references").mkdir(parents=True)
            (tdir / "shared/policy_data").mkdir(parents=True)
            shutil.copy(anchor, tdir / "hermes-academic-paper/references/policy_anchor_table.md")
            shutil.copy(venue, tdir / "hermes-academic-paper/references/venue_disclosure_policies.md")
            # Deliberately do NOT copy shared/policy_data/nature_policy.md
            exit_code = cpat.main(
                argv=[
                    str(tdir / "hermes-academic-paper/references/policy_anchor_table.md"),
                    "--venue-policies",
                    str(tdir / "hermes-academic-paper/references/venue_disclosure_policies.md"),
                ]
            )
            self.assertEqual(
