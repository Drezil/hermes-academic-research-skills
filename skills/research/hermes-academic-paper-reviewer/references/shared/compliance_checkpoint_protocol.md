...
**Used by**: `compliance_agent` (Task 8), `hermes-academic-pipeline/SKILL.md` Stage 2.5 / 4.5.
...
|---|---|---|
| `[MATERIAL GAP]` | `gaps[].reason`, `principle_evidence[]`, `evidence[]` | The item cannot be verified because the underlying material (passport field, manuscript section, supplementary file) is missing. Apply [Anti-Leakage Protocol](../hermes-academic-paper/references/anti_leakage_protocol.md) — do not hallucinate. |
...
When invoked from `hermes-academic-paper full` (primary research standalone), no pipeline state machine is available. Compliance_agent:
