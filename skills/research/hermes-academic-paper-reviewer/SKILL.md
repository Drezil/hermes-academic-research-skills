---
name: hermes-academic-paper-reviewer
...
# Academic Paper Reviewer — Multi-Perspective Review Team
...
|----------|-------------|
| Need to write a paper (not review) | `hermes-academic-paper` |
| Need in-depth investigation of a research topic | `hermes-deep-research` |
| Need to revise a paper (already have review comments) | `hermes-academic-paper` (revision mode) |
...
=== Phase 2: EDITORIAL SYNTHESIS & DECISION ===
     ...
     +-> [editorial_synthesizer_agent] -> Editorial Decision Package
         ...
         - Revision Roadmap (prioritized, can be directly input to hermes-academic-paper revision mode)
     |
     ...
     ** Only triggered when Decision = Minor/Major Revision **
     ...
     +-> [eic_agent] guides the user through Socratic dialogue:
         ...
         3. Contribution framing probe — ask the Layer-5 later-stage anchored forms
            ...
            hermes-deep-research/references/agents/socratic_mentor_agent.md — read the question text
...
hermes-academic-paper-reviewer runs in 3 phases internally (Phase 0 field analysis → Phase 1 panel review → Phase 2 editorial synthesis). Within the full ARS pipeline, this skill sits at the orchestrator's Phase 5 (Review), but each agent inside the reviewer skill is single-phase relative to the skill's own phase numbering.

Two invocation modes:

**Mode A — orchestrator-driven (default):** `pipeline_orchestrator_agent` (in `hermes-academic-pipeline` skill) dispatches `hermes-academic-paper-reviewer` as part of the full ARS pipeline Stage 3 (Review).

**Mode B — phase-by-phase (cross-session resume):** User invokes one reviewer agent per phase across sessions, or runs the full reviewer panel standalone via `hermes-academic-paper-reviewer` full mode.

In Mode B, **single-phase agents (Bucket A per `docs/design/2026-05-18-ars-v3.9.2-agent-phase-classification.md`) stay strictly within their assigned phase for writes**. The 6 Bucket A agents in hermes-academic-paper-reviewer are: `eic_agent`, `methodology_reviewer`, `domain_reviewer`, `perspective_reviewer`, `devils_advocate_reviewer` (all Phase 1 panel) + `editorial_synthesizer` (Phase 2 synthesis). Reading the full paper draft is **expected** for all reviewers — without context they cannot evaluate.
...
In `full` mode only (the five-seat panel — `methodology-focus` has a two-seat contract, and `re-review`/`quick` have no Reviewer 2 seat, so the track and its provenance mandate do not apply there), Reviewer 2 can optionally run on a different model. In the upstream Claude Code plugin this uses `ARS_CROSS_MODEL` with external API keys; in Hermes, enable **Mixture of Agents** (`hermes config set moa.enabled true`) to achieve the same cross-model diversity without external API setup. Otherwise all five personas share one model family — on the normal primary-family routing, including any active `ARS_MODEL_TIERING` policy — and the Editorial Decision Letter's Review Panel Provenance block discloses the correlated-error caveat (Ren et al. 2026, arXiv:2607.13104 §5.2). Dispatch failure falls back to that same primary-family routing with the fallback disclosed — never silent.
...
```
hermes-deep-research --> hermes-academic-paper --> [integrity check] --> hermes-academic-paper-reviewer --> hermes-academic-paper (revision) --> hermes-academic-paper-reviewer (re-review) --> [final integrity] --> finalize
...
|----------------------|-------------|
| **Upstream: hermes-academic-paper -> reviewer** | Receives the complete paper output from `hermes-academic-paper` full mode, directly enters Phase 0 |
| **Upstream: integrity check -> reviewer** | In the Pipeline, the paper must pass integrity check before entering reviewer |
| **Downstream: reviewer -> hermes-academic-paper** | The Revision Roadmap format can be directly used as reviewer feedback input for `hermes-academic-paper` revision mode |
...
|-------|----------------|
| field_analyst_agent | `references/agents/field_analyst_agent.md` |
| eic_agent | `references/agents/eic_agent.md` |
| methodology_reviewer_agent | `references/agents/methodology_reviewer_agent.md` |
| domain_reviewer_agent | `references/agents/domain_reviewer_agent.md` |
| perspective_reviewer_agent | `references/agents/perspective_reviewer_agent.md` |
| **devils_advocate_reviewer_agent** | **`references/agents/devils_advocate_reviewer_agent.md`** |
| editorial_synthesizer_agent | `references/agents/editorial_synthesizer_agent.md` |
...
| Skill | Relationship |
|-------|-------------|
| `hermes-academic-paper` | Upstream (provides paper) + Downstream (receives revision roadmap) |
| `hermes-deep-research` | Upstream (provides research foundation) |
| `tw-hei-intelligence` | Auxiliary (verifies higher education data accuracy) |
| `hermes-academic-pipeline` | Orchestrated by (Stage 3 + Stage 3') |
...
| Dependent Skills | hermes-academic-paper v1.0+ (upstream/downstream integration) |
