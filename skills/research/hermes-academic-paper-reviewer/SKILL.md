     ...
     ** Only triggered when Decision = Minor/Major Revision **
     ...
     +-> [eic_agent] guides the user through Socratic dialogue:
         ...
         3. Contribution framing probe — ask the Layer-5 later-stage anchored forms
            ...
            deep-research/references/agents/socratic_mentor_agent.md — read the question text
...
**Mode B — phase-by-phase (cross-session resume):** User invokes one reviewer agent per phase across sessions, or runs the full reviewer panel standalone via `academic-paper-reviewer` full mode.
