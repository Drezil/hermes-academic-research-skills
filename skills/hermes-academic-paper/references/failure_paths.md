...
This document records the failure scenarios that the hermes-academic-paper skill may encounter at each stage, their trigger conditions, and handling strategies. All agents should refer to this guide when they detect a failure scenario.
...
|---|---------|---------|--------|---------|
| F1 | Insufficient research foundation | Plan mode Step 0 finds no RQ / no data | High | Recommend running `hermes-deep-research` first |
...
| F9 | Incomplete handoff materials | From hermes-deep-research but missing key materials | Low | List missing items, suggest supplementing or re-running |
...
3. Recommend using hermes-deep-research (socratic mode)
4. Explain that they can come back to continue after hermes-deep-research is completed
...
I recommend you first use the hermes-deep-research tool to:
...
**Trigger Timing**: intake_agent detects hermes-deep-research materials but they are incomplete
...
3. Recommend:
   a. Return to hermes-deep-research to complete the missing parts
   b. Or supplement within hermes-academic-paper (add Phase 0 interview questions)
```
...
**How to Resume**: Bring this record and restart hermes-academic-paper; can continue from Phase {N}
```
...
```
F1 (Insufficient research foundation) → Recommend hermes-deep-research → May encounter F9 (incomplete materials) upon return
...
| **Insufficient Novelty** | "Incremental contribution" or "well-established findings" | Sharpen the contribution positioning WITHIN the documented search bounds (#548): enumerate the nearest prior work and state the delta precisely; consider additional analysis or a new dataset; never respond by absolutizing novelty language ("first study to...") beyond what the documented search supports |
