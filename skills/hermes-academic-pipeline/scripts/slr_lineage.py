...
Pipeline plumbing helper for the `hermes-deep-research systematic-review →
hermes-academic-paper full → disclosure --policy-anchor=prisma-trAIce` auto-
...
def resolve_from_stages(stages: Mapping[str, Mapping]) -> bool:
    """Return True iff any stage was produced by hermes-deep-research in SLR mode.

    Run-level provenance: the contract is bound to hermes-deep-research lineage
    specifically, mirroring the §4.3 G2 invariant track gate. A
    non-hermes-deep-research stage with mode='systematic-review' does NOT
    trigger SLR lineage — only the documented producer counts.
    """
    return any(
        (stage.get("skill") == "hermes-deep-research")
